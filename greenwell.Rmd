...
* Discuss advantages and disadvantages described in \url{https://christophm.github.io/interpretable-ml-book/shapley.html}.
* Mention the technical difference between SHAP (which satisfies the consistency property) and traditional Shapley values (like SampleSHAP), and how we loosen the distinction of the terminology here.
...
\end{equation}
where $\mathcal{O}$ is a specific permutation of the players indices $\left\{1, 2, \dots, p\right\}$, $\pi\left(p\right)$ is the set of all suck permutations of size $p$, and $S^\mathcal{O}$ is the set of players joining the coalition before player $i$.
