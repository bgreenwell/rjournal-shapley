...
\end{split}
\end{equation}
where $S^c$ is the complement $S$, $x_S$ and $x_{S^c}$ are the set of features in $S$ and $S^c$, respectively, and $p\left(x_{S^c}\right)$ is the joint probability density of $x_{S^c}$. Equation \eqref{eqn:ice} can be interpreted as the expected value of $f\left(x\right)$ given some intervention on the features in $S$, which assumes independence between $x_S$ and $x_{S^c}$. Shapley values based on this formulation of $v$ are referred to as \dfn{interventional Shapley values} \citep{chen-2020-true}. The various Shapley value algorithms discussed over the next several sections fall under this form.
...
gbm.trn <- gbm(ifelse(credit.trn$default == "yes", 1, 0) ~ ., data = credit.trn, 
               distribution = "bernoulli", n.trees = 300, interaction.depth = 5, 
               shrinkage = 0.1, cv.folds = 5)
par(las = 1)  # horizontal y-axis labels
(best <- gbm.perf(gbm.trn, plot.it = TRUE, method = "cv"))
```

Finish later...

```{r ex-credit-gbm-predict, fig.cap="TBD."}
p <- predict(gbm.trn, newdata = credit.tst, n.trees = "best")
max(p)
pfun <- function(object, newdata) {
  predict(object, newdata = newdata, n.trees = "best")
}
predictor <- iml::Predictor$new(gbm.trn, data = credit.trn, y = "default",
                                predict.fun = pfun)
...
<!-- ----------------------------------------------------------------------- -->
\subsection{A simple benchmark comparison}
<!-- ----------------------------------------------------------------------- -->
...
```{r titanic-jack-explanations-plot, fig.width=9, fig.height=3, out.width="100%", fig.cap="TBD."}
library(ggplot2)
