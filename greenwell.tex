...
\item
  ...
  consistency property) and Shapley values, and how we loosen the
  distinction of the terminology here.
\end{itemize}
...
\(S\), which assumes independence between \(x_S\) and \(x_{S^c}\).
Shapley values based on this formulation of \(v\) are referred to as
...
which we'll call SampleSHAP\^{}footnote\{\textbf{FIXME:} Make a note on
the technical use of the term SHAP, and how we're being loose with the
terminology here.\}, that assumes independent
...
{\centering \includegraphics[width=1\linewidth]{greenwell_files/figure-latex/ex-credit-gbm-predict-1} 

}

\caption[TBD]{TBD.}\label{fig:ex-credit-gbm-predict}
\end{figure}
\end{Schunk}
